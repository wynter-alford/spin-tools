...
testVarName = 'TauDeltaGrid'; % only affects file name currently
testValueCount = 30; 
%testValueMax = 30e-6;
...
Deltas = zeros(ll,1);
couplings = zeros(ll,1);

results_h0 = zeros(ll,ll);
results_h1 = zeros(ll,ll);
results_h2 = zeros(ll,ll);
results_h3 = zeros(ll,ll);
results_h4 = zeros(ll,ll);

raw_results_h0 = zeros(ll,ll,couplingsCount);
raw_results_h1 = zeros(ll,ll,couplingsCount);
raw_results_h2 = zeros(ll,ll,couplingsCount);
raw_results_h3 = zeros(ll,ll,couplingsCount);
raw_results_h4 = zeros(ll,ll,couplingsCount);


for d=1:ll
    Delta = d*(2000/testValueCount); % ADJUST FOR TEST VAR ################################
    %testVars(d)=Delta; % ADJUST FOR TEST VAR ######################################################


    for cp =1:ll
        coupling = cp*(4000/testValueCount);
        ...
        for c=1:couplingsCount
            ...
            raw_results_h0(d,cp,c) = h0;
            raw_results_h1(d,cp,c) = h1;
            raw_results_h2(d,cp,c) = h2;
            raw_results_h3(d,cp,c) = h3;
            raw_results_h4(d,cp,c) = h4;
        end

        results_h0(d,cp)=mean(raw_results_h0(d,cp,:));
        results_h1(d,cp)=mean(raw_results_h1(d,cp,:));
        results_h2(d,cp)=mean(raw_results_h2(d,cp,:));
        results_h3(d,cp)=mean(raw_results_h3(d,cp,:));
        results_h4(d,cp)=mean(raw_results_h4(d,cp,:));
    end
end
...
save(strcat('taus_',fileDescriptor),'Taus');
save(strcat('deltas_',fileDescriptor),'Deltas');
save(strcat('couplings_',fileDescriptor),'couplings');
